...
import model.exception.IllegalDimensionException;
...
public class MatrixMathematics {
     ...
     * @param matrix
     * @return tab de BigDecimal
     * @throws NoSquareException
     */
    public static BigDecimal[][] inverse(Matrix matrix) throws NoSquareException {
        BigDecimal [][] matFloat = new BigDecimal[matrix.size()][matrix.size()];
        Matrix matrix2 = transpose(cofactor(matrix));
        //System.out.println("det "+determinant(matrix));
        BigDecimal det = new BigDecimal(determinant(matrix));
        for (int i=0;i<matrix2.getNrows();i++) {
            for (int j = 0; j < matrix2.getNcols(); j++) {
                matFloat[i][j]= new BigDecimal(matrix2.getValueAt(i, j));
                //System.out.println("avant: "+matFloat[i][j]);
                matFloat[i][j]=matFloat[i][j].divide(det,10, RoundingMode.CEILING);
               //System.out.println("apres: "+matFloat[i][j]);
            }
        }

       return matFloat;//(transpose(cofactor(matrix)).multiplyByConstant(new BigInteger("1").divide(determinant(matrix))));
    }
    ...
    public static BigInteger determinant(Matrix matrix) throws NoSquareException {
        ...
        for (int i=0; i<matrix.getNcols(); i++) {
            sum = sum.add(BigInteger.valueOf(changeSign(i)).multiply(matrix.getValueAt(0, i)).multiply(determinant(createSubMatrix(matrix, 0, i))));
        }
        return sum;
    }
    ...
    public static Matrix cofactor(Matrix matrix) throws NoSquareException {
        ...
        for (int i=0;i<matrix.getNrows();i++) {
            for (int j=0; j<matrix.getNcols();j++) {
                mat.setValueAt(i, j, BigInteger.valueOf(changeSign(i)).multiply(BigInteger.valueOf(changeSign(j))).multiply(determinant(createSubMatrix(matrix, i, j))));
            }
        }
    ...
    public static Matrix add(Matrix matrix1, Matrix matrix2) throws IllegalDimensionException {
        ...
        for (int i=0; i<matrix1.getNrows();i++) {
            for (int j=0;j<matrix1.getNcols();j++)
                sumMatrix.setValueAt(i, j, matrix1.getValueAt(i, j).add(matrix2.getValueAt(i, j)));
    ...
    public static Matrix subtract(Matrix matrix1, Matrix matrix2) throws IllegalDimensionException {
        return add(matrix1,matrix2.multiplyByConstant(BigInteger.valueOf(-1)));
    }
        ...
        for (int i=0;i<multipliedMatrix.getNrows();i++) {
            for (int j=0;j<multipliedMatrix.getNcols();j++) {
                ...
                for (int k=0;k<matrix1.getNcols();k++) {
                    sum = sum.add(matrix1.getValueAt(i, k).multiply(matrix2.getValueAt(k, j)));
                }
